...
function InputCard(props:InputValueType){
    const{inputValueName , inputValueNumber , inputValueMonth , inputValueYear , inputValueCvc ,setInputValueName,setInputValueNumber ,setInputValueMonth ,setInputValueYear ,setInputValueCvc ,setIsVisible} = props
    ...
    return(
        ...
        <form className="form-div">
          ...
          <input
            ...
            onChange={(e) => {if(e.target.value.length <=25){
          ...
          />
          {!input2Ref.current?.value || input2Ref.current?.value.length===16  ? null : <p className="error1">Wrong format, numbers only . Must Be 16 Digits</p>}
          ...
          {!input3Ref.current?.value && !input4Ref.current?.value && !input5Ref.current?.value || input3Ref.current?.value.length ===2 &&  input4Ref.current?.value.length ===2 && input5Ref.current?.value.length ===3 ? null : <p className='blank'>Can’t be blank</p> }
        </form>
        <button onClick={(e) => {if(input1Ref.current?.value.length >=5 && input2Ref.current?.value.length===16 && input3Ref.current?.value.length ===2 && input4Ref.current?.value.length ===2 && input5Ref.current?.value.length ===3){setIsVisible(false);}}}  className='button'>Confirm</button>
        </div>
    );
}
